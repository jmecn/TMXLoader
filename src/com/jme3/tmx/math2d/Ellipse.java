...
public class Ellipse implements Shape2D {
	 ...
	 * @param width Width in pixels
	 * @param height Height in pixels */
	public Ellipse (Vector2f position, float width, float height) {
		this.x = position.x;
		this.y = position.y;
		this.width = width;
		this.height = height;
	...
	public Ellipse (Vector2f position, Vector2f size) {
		this.x = position.x;
		this.y = position.y;
		this.width = size.x;
		this.height = size.y;
	...
	/** Sets the x and y-coordinates of ellipse center from a {@link Vector2f}.
	...
	/** @return The area of this {@link Ellipse} as {@link FastMath#PI} * {@link Ellipse#width} * {@link Ellipse#height} */
	public float area () {
		return FastMath.PI * (this.width * this.height) / 4;
	}
	...
	public float circumference () {
		...
		if (a * 3 > b || b * 3 > a) {
			...
			return (float)(FastMath.PI * ((3 * (a + b)) - Math.sqrt((3 * a + b) * (a + 3 * b))));
		} else {
			// We can use the simpler approximation, then
			return (float)(FastMath.TWO_PI * Math.sqrt((a * a + b * b) / 2));
		}
	}
